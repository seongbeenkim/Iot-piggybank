...
from pigmong_audio import AudioFormat, Recorder
...
class CloudSpeechClient:
    def __init__(self, service_accout_file=None):
        if service_accout_file is None:
            service_accout_file = os.path.expanduser('/home/pi/cloud_speech.json')
        ...
        with Recorder() as recorder:
            ...
            for response in responses:
                if response.speech_event_type == END_OF_SINGLE_UTTERANCE:
                    recorder.done()
                    ####
                    i2cpig.writeNumber(Arduino_address,20) # exit - led_listening
                    time.sleep(0.96)
                    i2cpig.writeNumber(Arduino_address,12) # led_processing
                    ####
                for result in response.results:
                    if result.is_final:
                        return result.alternatives[0].transcript
    ...
    def start_listening(self):
        ####
        i2cpig.writeNumber(Arduino_address,11) # led_listening
        ####
