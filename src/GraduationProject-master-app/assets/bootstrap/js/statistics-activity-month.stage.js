	...
	if ($('#interactive-chart-stage').length !== 0) {
        ...
        $.plot($("#interactive-chart-stage"), [
            ...
            ],
            {
                ...
                yaxis: {  ticks: 10, tickColor: '#ddd', min: 0, max: 5000 },
                grid: {
                    hoverable: true,
                    clickable: true,
                    tickColor: "#ddd",
...
var handleInteractiveChartStage2 = function (options) {
	"use strict";
    function showTooltip(x, y, contents) {
        $('<div id="tooltip" class="flot-tooltip">' + contents + '</div>').css( {
            top: y - 45,
	...
	if ($('#interactive-chart-stage-2').length !== 0) {

        $.plot($("#interactive-chart-stage-2"), [
                {
                    data:  options.data1,
                    label: "스테이지 참여회원",
                    color: blue,
        ...
        var previousPoint = null;
        $("#interactive-chart-stage-2").bind("plothover", function (event, pos, item) {
            $("#x").text(pos.x);
            $("#y").text(pos.y);
            if (item) {
                if (previousPoint !== item.dataIndex) {
...
var StatisticsActivityMonthStage = function () {
	"use strict";
    return {
        //main function
        init: function (options, options2) {
            handleInteractiveChartStage(options);
            handleInteractiveChartStage2(options2);
        }
    };
}();
